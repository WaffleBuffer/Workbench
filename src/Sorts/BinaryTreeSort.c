 ...
 * @author Thomas MEDARD, Remi SEGRETAIN
 */
void insert (struct BTreeNode **node, int value) {
    if (*node == NULL) {
 ...
 * @author Thomas MEDARD, Remi SEGRETAIN
 */
void inorder (const struct BTreeNode *node, int tab[]){
    if (node != NULL){
